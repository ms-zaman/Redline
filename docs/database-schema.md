...
This document outlines the PostgreSQL database schema with PostGIS extension for the Bangladesh Political Violence Tracker.

## Overview

The database is designed to efficiently store and query:

-   Raw news articles from various sources
-   AI-classified political violence incidents
-   Geographic data with spatial indexing
-   Source configurations and metadata
...
Configuration for news websites to scrape.
...
Raw scraped news articles before AI processing.
...
AI classification results for articles.
...
CREATE INDEX idx_classifications_is_political_violence ON classifications(is_political_violence);
...
Geographic reference data for Bangladesh.
...
CREATE TABLE locations (
    ...
    type VARCHAR(50) NOT NULL, -- division, district, upazila, union, village
    parent_id INTEGER REFERENCES locations(id),
    admin_code VARCHAR(20), -- Official administrative code
...
Extracted political violence incidents.
...
-- Indexes
CREATE INDEX idx_incidents_article_id ON incidents(article_id);
...
CREATE INDEX idx_incidents_casualties ON incidents(killed, injured);
CREATE INDEX idx_incidents_verified ON incidents(verified);
CREATE INDEX idx_incidents_created_at ON incidents(created_at);

-- GIN indexes for array fields
CREATE INDEX idx_incidents_perpetrators ON incidents USING GIN(perpetrators);
CREATE INDEX idx_incidents_victims ON incidents USING GIN(victims);
CREATE INDEX idx_incidents_political_parties ON incidents USING GIN(political_parties);
```
...
CREATE TABLE processing_logs (
    id SERIAL PRIMARY KEY,
    article_id INTEGER REFERENCES articles(id),
    process_type VARCHAR(50) NOT NULL, -- scraping, classification, extraction, geocoding
...
-- Indexes
CREATE INDEX idx_processing_logs_article_id ON processing_logs(article_id);
...
-- Indexes
CREATE INDEX idx_scraping_sessions_source_id ON scraping_sessions(source_id);
...
Aggregated view for map display.
