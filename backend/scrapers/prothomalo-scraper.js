...
class ProthomAloScraper {
    constructor() {
        this.baseUrl = "https://www.prothomalo.com";
        this.name = "Prothom Alo";
        this.language = "bn"; // Bengali
        ...
        this.requestConfig = {
            headers: {
                ...
                "Accept-Language": "bn,en-US,en;q=0.5",
    ...
    async getLatestNewsUrls(limit = 10) {
        try {
            ...
            const sectionUrls = [
                `${this.baseUrl}/politics`, // রাজনীতি
                `${this.baseUrl}/bangladesh`, // বাংলাদেশ
                `${this.baseUrl}/bangladesh/crime`, // অপরাধ
                `${this.baseUrl}/collection/latest`, // সর্বশেষ
            ];
                ...
                try {
                    ...
                    $("a").each((index, element) => {
                        ...
                        if (href) {
                            ...
                            if (
                                this.isArticleUrl(fullUrl) &&
                                !newsUrls.includes(fullUrl)
                            ) {
                    ...
                    console.error(
                        `Error fetching from ${sectionUrl}:`,
                        error.message
                    );
                }
            ...
            // If we still don't have enough, try the homepage
            if (newsUrls.length === 0) {
                ...
                $(
                    'a[href*="/politics/"], a[href*="/bangladesh/"], a[href*="/opinion/"]'
                ).each((index, element) => {
                    const href = $(element).attr("href");
     ...
     */
    isArticleUrl(url) {
        // Prothom Alo article URL patterns - they use 10-character alphanumeric IDs
        const hasArticleIndicators = [
            /\/[a-z0-9]{10}$/, // Ends with 10-character alphanumeric ID
            /\/(politics|bangladesh|opinion|world|business)\/[a-z0-9]{10}$/,
            /\/(politics|bangladesh|opinion)\/[^\/]+\/[a-z0-9]{10}$/,
            /\/(bangladesh\/district|bangladesh\/crime)\/[a-z0-9]{10}$/,
        ];

        const excludePatterns = [
            /\/(politics|bangladesh|opinion|world|business|sports|entertainment|chakri|lifestyle)\/?$/, // Section pages
            /\/collection\//,
            /\/search/,
            /\/author\//,
            /\/category\//,
        ...
        const hasIndicator = hasArticleIndicators.some((pattern) =>
            pattern.test(url)
        );
    ...
    async scrapeArticle(url) {
        try {
            ...
            for (const selector of titleSelectors) {
                ...
                if (titleEl.length > 0) {
                    title = titleEl.text().trim().replace(" | প্রথম আলো", "");
            ...
            for (const url of newsUrls) {
                try {
                    ...
                    if (
                        ...
                        console.log(
                            `⚠️  Skipping article with insufficient content: ${url}`
                        );
                        results.failed++;
                        continue;
                    }
                    ...
                    console.log(
                        `✅ Saved: ${articleData.title.substring(0, 60)}... (${
...
export default ProthomAloScraper;
