...
class AmarDeshScraper {
    constructor() {
        this.baseUrl = "https://www.dailyamardesh.com";
        this.name = "Daily Amar Desh";
    ...
    async getLatestNewsUrls(limit = 10) {
        try {
            ...
            const sectionUrls = [
                `${this.baseUrl}/national`,      // জাতীয়
                `${this.baseUrl}/politics`,      // রাজনীতি
                `${this.baseUrl}/bangladesh`,    // সারা দেশ
                `${this.baseUrl}/july-revolution` // জুলাই বিপ্লব
            ];
            ...
            if (newsUrls.length === 0) {
                ...
                $('a[href*="/national/"], a[href*="/politics/"], a[href*="/bangladesh/"], a[href*="/july-revolution/"]').each((index, element) => {
                    const href = $(element).attr('href');
     ...
     */
    isArticleUrl(url) {
        // Amar Desh article URL patterns
        const hasArticleIndicators = [
            /\/amd[a-z0-9]+$/,                    // Ends with amdXXXXXX (article ID)
            /\/(national|politics|bangladesh|july-revolution|world|business)\/amd[a-z0-9]+$/,
            /\/(national|politics|bangladesh|july-revolution)\/[^\/]+\/amd[a-z0-9]+$/
        ];
        
        const excludePatterns = [
            /\/(national|politics|bangladesh|july-revolution|world|business)\/?$/,  // Section pages
            /\/tags\//,
            /\/search\//,
            /\/author\//,
            /\/category\//,
    ...
    async scrapeArticle(url) {
        try {
            ...
            for (const selector of titleSelectors) {
                ...
                if (titleEl.length > 0) {
                    title = titleEl.text().trim().replace(' | আমার দেশ', '');
...
export default AmarDeshScraper;
